(function(){
  ...
  if(darkBtn){ darkBtn.addEventListener('click', ()=>{ const cur = localStorage.getItem('theme') || 'auto'; const next = cur==='auto' ? 'dark' : (cur==='dark' ? 'light' : 'auto'); localStorage.setItem('theme', next); applyTheme(); });
  }
  applyTheme();
  ...
  function buildPlan(){
    ...
    const payload = {
      ...
      renal:{crcl}, meds:{tnkMg:tnk, asa:asaPlan, clopidogrel:clop, anticoagulation:anticoag}, schedule, adminRecords,
      evoNotes: {inicio: localStorage.getItem('evo_inicio')?JSON.parse(localStorage.getItem('evo_inicio')):'', intermedia: localStorage.getItem('evo_intermedia')?JSON.parse(localStorage.getItem('evo_intermedia')):'', final: localStorage.getItem('evo_final')?JSON.parse(localStorage.getItem('evo_final')):''}
    };
